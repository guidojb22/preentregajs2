...
let categoria1 = "women's clothing";
let categoria2 = "jewelery";

function filtroCategorias(nuevoArray, categoria1, categoria2) {
    return nuevoArray.filter (productoFiltrado  => productoFiltrado.category == categoria1  || productoFiltrado.category == categoria2)
}

let productosCategorias = filtroCategorias (products, categoria1, categoria2);
...
alert("En la siguiente ventana aparecerán nuestros productos. Si querés alguno en especial, escribí el nombre EXACTO del mismo respetando mayúsculas y minúsculas.");

console.log (productosCategorias.sort(( a , b ) => {
    if ( a.title < b.title ) {
        return -1
...
const nombres = productosCategorias.reduce (( acc, elementoActual, indice ) => acc + `\n${indice + 1}) ${elementoActual.title}`, "");

let eleccionProducto = prompt("¿Qué producto querés comprar?\n" + nombres);

const productoElegido = productosCategorias.find (nombre => nombre.title === eleccionProducto); 
